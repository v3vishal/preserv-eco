...
window.onload = function () {
  ...
  JSC.fetch(
      'https://raw.githubusercontent.com/v3vishal/preserv-eco/main/res/stcforcov.csv'
  )
      ...
      .then(function (text) {
        ...
        chart = renderChart(makeSeries(data, '2011')); // Initial chart with 2019 data
      })
  ...
  function makeSeries(data, year) {
    return JSC.nest()
      ...
      .pointRollup(function (key, val) {
        var value = val[0],
          datatyp = value.datatype === 'Forest Cover'
        return {
          x: key,
          id: key + ' ' + value.datatype,
          y: value['year_' + year],
          label_align: datatyp ? 'left' : 'right'
        };
      })
      .series(data);
  }
  */
        ...
        JSC.fetch('https://raw.githubusercontent.com/v3vishal/preserv-eco/main/res/stcforcov.csv')
          .then(function(response) {
            return response.text();
          })
          .then(function(text) {
            data = JSC.csv2Json(text);
            chart = renderChart(makeSeries(data, 2011));
          })
          .catch(function(error) {
            console.error(error);
          });
